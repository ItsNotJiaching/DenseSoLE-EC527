...
int main() {
  ...
  int OPTION = 0;
  // CUDA Implementation, single kernel, block size 16
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    sole_cuda(arrA_copy, arrX, arrB_copy, n, 16);
    clock_gettime(CLOCK_MONOTONIC, &time_stop);
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 32
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
    ...
    printf(" Option %d, iter %ld, size %ld\n", OPTION, x, n);
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    sole_cuda(arrA_copy, arrX, arrB_copy, n, 32);
    clock_gettime(CLOCK_MONOTONIC, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    error[OPTION][x] = verify(arrA, arrX, arrB,n);
  }
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 64
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
    ...
    printf(" Option %d, iter %ld, size %ld\n", OPTION, x, n);
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    sole_cuda(arrA_copy, arrX, arrB_copy, n, 64);
    clock_gettime(CLOCK_MONOTONIC, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    error[OPTION][x] = verify(arrA, arrX, arrB,n);
  }
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 128
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
    ...
    printf(" Option %d, iter %ld, size %ld\n", OPTION, x, n);
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    sole_cuda(arrA_copy, arrX, arrB_copy, n, 128);
    clock_gettime(CLOCK_MONOTONIC, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    error[OPTION][x] = verify(arrA, arrX, arrB,n);
  }
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 256
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 512
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
    ...
    printf(" Option %d, iter %ld, size %ld\n", OPTION, x, n);
    clock_gettime(CLOCK_MONOTONIC, &time_start);
    sole_cuda(arrA_copy, arrX, arrB_copy, n, 512);
    clock_gettime(CLOCK_MONOTONIC, &time_stop);
    time_stamp[OPTION][x] = interval(time_start, time_stop);
    error[OPTION][x] = verify(arrA, arrX, arrB,n);
  }
  ...
  OPTION++;
  // CUDA Implementation, single kernel, block size 1024
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=arr_len); x++) {
    //copy originals
    data_t* arrA_copy = (data_t*) calloc(arr_len * arr_len, sizeof(data_t)); // copy for error check
    data_t* arrB_copy = (data_t*) calloc(arr_len, sizeof(data_t)); // copy for error check
  ...
  printf("row_len, "
        ...
        "block32, block32_err, "
        "block64, block64_err, "
        "block128, block128_err, "
        "block256, block256_err, "
        "block512, block512_err, "
        "block1024, block1024_err\n");
